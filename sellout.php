...
// 1. ตรวจสอบว่าเริ่ม session แล้วหรือยัง
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

// *** โค้ดป้องกันการเข้าถึงโดยตรงด้วย Session Token ***

$allowed_token = $_SESSION['SELLOUT_ACCESS_TOKEN'] ?? null;
$received_token = $_GET['token'] ?? null;

// 1. ตรวจสอบสถานะการเข้าสู่ระบบ (สมมติว่ามีการตั้งค่า $_SESSION['loggedin'] ในไฟล์ index.php หรือ login)
...
// 2. ตรวจสอบ Token (ต้องมี token และตรงกับ session token)
// เราสมมติว่า index.php หรือ employee_dashboard.php เป็นไฟล์ที่สร้าง Token นี้
if (empty($received_token) || $received_token !== $allowed_token) {
    // ถ้าเข้าถึงโดยตรงหรือ Token ไม่ถูกต้อง/หมดอายุ ให้เปลี่ยนเส้นทาง
    
    // ลบ Token ออกจาก Session เพื่อป้องกันการนำ URL ที่มี Token ไปใช้ซ้ำ
    unset($_SESSION['SELLOUT_ACCESS_TOKEN']); 
    
    // เปลี่ยนเส้นทางไปยังหน้าหลัก/Dashboard
    // เปลี่ยนเส้นทางไปที่ index.php แทน employee_dashboard.php ตามคำขอ
    header("Location: index.php"); 
    exit;
}

// 3. เมื่อเข้าถึงสำเร็จ ให้ลบ Token ทันทีเพื่อความปลอดภัย (Token ใช้งานได้ครั้งเดียว)
unset($_SESSION['SELLOUT_ACCESS_TOKEN']);

// *** สิ้นสุด: โค้ดป้องกันการเข้าถึงโดยตรง ***
